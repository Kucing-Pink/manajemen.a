...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Optimization: Skeleton Loaders ---
  function showSkeletons(containerId, count = 4, className = 'highlight-card') {
    ...
    for (let i = 0; i < count; i++) {
      ...
      skeleton.style.height = className === 'media-item' ? '250px' : '180px';
      container.appendChild(skeleton);
    }
  }

  // --- Optimization: Intersection Observer for Videos ---
  ...
  function renderHighlights() {
    ...
    topMemories.forEach((item, index) => {
      let uploaderHp = String(item.hp || '').replace(/\D/g, '');
      let uploader = window.GlobalProfiles[uploaderHp] || { name: 'Keluarga Manajemen A', avatar: 'https://i.pravatar.cc/150?img=11' };
      const card = document.createElement('div');
      card.className = 'highlight-card';
      const fetchPriority = index < 2 ? 'fetchpriority="high"' : '';
      card.innerHTML = `
        <div class="uploader-badge">
          <img src="${uploader.avatar}" class="mini-avatar">
          <span>${uploader.name}</span>
        </div>
        <img src="${item.url}" alt="${item.caption}" loading="lazy" ${fetchPriority}>
        <div class="overlay">
  ...
  function renderGallery() {
    ...
    filtered.forEach(item => {
      ...
      let mediaTag = item.type === 'video' 
        ? `<video src="${item.url}" muted loop playsinline preload="none" style="width: 100%; display: block; object-fit: cover;"></video>`
      ...
      if (item.type === 'video') {
        videoObserver.observe(el.querySelector('video'));
      }
  ...
  async function fetchGlobalMemories() {
    ...
    try {
      ...
      if(Array.isArray(json) && json.length > 0) {
          memories = json.map(j => {
            const getVal = (key) => {
              const k = Object.keys(j).find(k => k.toLowerCase() === key.toLowerCase());
              return k ? j[k] : '';
            };
            
            let rawUrl = getVal('url') || getVal('link') || getVal('src') || getVal('fileurl') || '';
            if (rawUrl.includes('drive.google.com') && !rawUrl.includes('uc?export=view')) {
              const idMatch = rawUrl.match(/[-\w]{25,}/);
              if (idMatch) rawUrl = `https://drive.google.com/uc?export=view&id=${idMatch[0]}`;
            }

            return {
              id: getVal('id') || Date.now(),
              type: getVal('type') || (rawUrl.includes('video') ? 'video' : 'image'),
              url: rawUrl,
              caption: getVal('caption') || '',
              likes: getVal('likes') || 0,
              comments: getVal('comments') || 0,
              date: getVal('date') || new Date().toISOString().split('T')[0],
              semester: getVal('semester') || '1',
              hp: String(getVal('hp') || getVal('uploader') || '').replace(/\D/g, '') // Normalize HP
            };
          }).reverse();
      }
    } catch(e) {
  ...
  async function initializeApp() {
    showSkeletons('topMomentsContainer', 4, 'highlight-card');
    showSkeletons('galleryGrid', 6, 'media-item');
    
    await fetchGlobalProfiles();
  ...
  submitUploadBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      let uploadBase64 = currentBase64;
      
      if(currentItemType === 'image') {
        uploadBase64 = await compressImage(currentBase64);
      }
      
      const base64Data = uploadBase64.split(',')[1];
      const mimeType = currentItemType === 'image' ? 'image/jpeg' : currentUploadFile.type;
      ...
      if(result.error) throw new Error(result.error);
       ...
       const newMemory = { id: Date.now(), type: currentItemType, url: currentBase64, caption: cap, likes: '0', comments: '0', date: currentDate, semester: sem, hp: currentUserHp };
  ...
  function simulateUpload(cap, sem) {
       const newMemory = { id: Date.now(), type: currentItemType, url: currentBase64, caption: cap, likes: '0', comments: '0', date: new Date().toISOString().split('T')[0], semester: sem, hp: currentUserHp };
      memories.unshift(newMemory);
      renderHighlights();
      renderGallery();
      resetUploadModal();
  ...
  window.openMediaModal = function(item) {
    ...
    // Update Modal Header with Uploader
    const modalAvatar = document.querySelector('.media-author .avatar-img');
    const modalName = document.querySelector('.media-author h4');
    if(modalAvatar) modalAvatar.src = uploader.avatar;
    if(modalName) modalName.textContent = uploader.name;

    if(item.type === 'video') {
       mediaDisplayArea.innerHTML = `<div class="loading-spinner"></div><video src="${item.url}" autoplay controls playsinline style="width: 100%; height: 100%; object-fit: contain; background: #000;" onloadeddata="this.previousSibling?.remove()"></video>`;
    } else {
       mediaDisplayArea.innerHTML = `<div class="loading-spinner"></div><img src="${item.url}" alt="Kenangan" style="width: 100%; height: 100%; object-fit: contain; background: #000;" onload="this.previousSibling?.remove()" onerror="this.src='https://placehold.co/1200x800?text=Maaf,+Kenangan+Sedang+Malu-malu'; this.previousSibling?.remove();">`;
    }
  ...
  function renderCommentsForCurrentMemory() {
    ...
    if(memoryComments.length === 0) {
      commentsList.innerHTML = '<div class="empty-state">Jadilah yang pertama untuk meninggalkan pesan hangat! ✨</div>';
      return;
    }
    
    memoryComments.forEach(c => {
      let userProfile = window.GlobalProfiles[c.hp] || { name: 'Anonim', avatar: 'https://i.pravatar.cc/150?img=1' };
      
      const div = document.createElement('div');
      div.className = 'comment-bubble';
      div.innerHTML = `
        <img src="${userProfile.avatar}" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid var(--accent-secondary);">
        <div class="comment-text-wrapper">
          <div class="comment-author">
             <span>${userProfile.name}</span>
             <span class="comment-date">${new Date(c.date).toLocaleDateString('id-ID', {day: 'numeric', month: 'short'})}</span>
          </div>
          <p class="comment-msg">${c.text}</p>
        </div>
      `;
